...
export async function writeDay(day) {
    const file = filePath(day.date)

    await fs.mkdir(path.dirname(file), { recursive: true })
    await fs.writeFile(file, JSON.stringify(day))
}
...
function filePath(date) {
    ...
    return path.join(directory, year, month, `${day}.json`)
}

function dateOf(file) {
    return file.replace(/\.json$/, '').split(path.sep).join('-')
}
