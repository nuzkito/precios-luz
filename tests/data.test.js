...
describe('writeDay', () => {
    it('saves the day as JSON in the history, creating the folder', async () => {
...
describe('listDates', () => {
    it('gives the days of the history in order', async () => {
        await fs.mkdir(directory, { recursive: true })

        for (const date of ['2026-07-30', '2025-01-02', '2026-07-28']) {
            await fs.writeFile(`${directory}/${date}.json`, '{}')
        }

        assert.deepEqual(await listDates(), ['2025-01-02', '2026-07-28', '2026-07-30'])
    })

    it('leaves out whatever is not a day', async () => {
        await fs.mkdir(directory, { recursive: true })
        await fs.writeFile(`${directory}/2026-07-29.json`, '{}')
        await fs.writeFile(`${directory}/2026-02-30.json`, '{}')
